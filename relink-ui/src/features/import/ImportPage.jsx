...
export default function ImportPage() {
  ...
  async function onFilesSelected(ev) {
    ...
    const mapped = await Promise.all(files.map(toRow));
    // ZAMIEŃ to co masz teraz na dokładnie to:
const [rows, setRows] =
  /** @type {[UITrack[], (v: UITrack[] | ((p: UITrack[]) => UITrack[])) => void]} */ (
    useState([])
  );
  ...
  async function deleteSelected() {
    ...
    setRows(prev => prev.filter(r => !ids.includes(r.id))); // lokalnie
  ...
  async function createPlaylist(name) {
    const uris = rows.filter(r => r.added && r.spotifyId)
